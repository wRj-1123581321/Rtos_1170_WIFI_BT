...
#ifdef CONFIG_WIFI_GSD_TEST

/** A const buffer to send from: we want to measure sending, not copying! */
static const u8_t gsd_txbuf_const[2000] = {
...
static void gsd_udp_send(int argc, char **argv)
{
	ip_addr_t DestIPaddr, local_addr;
	err_t err;
	u32_t send_count = 0;
	struct udp_pcb *upcb;

	(void)memset(&DestIPaddr, 0, sizeof(DestIPaddr));
	(void)memset(&local_addr, 0, sizeof(local_addr));
	/* 设置服务器端的IP地址 */
	//IP4_ADDR( &DestIPaddr, 192, 168, 1, 2);
	(void)inet_aton("192.168.1.2", ip_2_ip4(&DestIPaddr));
	(void)inet_aton("192.168.1.1", ip_2_ip4(&local_addr));
	...
	if (argc == 1) {
		...
		send_count = strtol(argv[1], NULL, 10);
	} else {
	...
	upcb = udp_new();
	if (upcb!=NULL) {
		err = udp_bind(upcb, &local_addr, 0);
		...
		if (err == ERR_OK) {
			...
			for (int i=0; i<send_count; i++) {
				/* 分配内存空间 */
				p = pbuf_alloc(PBUF_TRANSPORT, 1470, PBUF_POOL);
				if (p != NULL) {
					/* 复制数据到pbuf */
					pbuf_take(p, gsd_txbuf_const, 1470);
					/* 发送数据 */
...
#ifdef CONFIG_WIFI_GSD_TEST  
    {"udp", "[-s|-c <host>|-a|-h] [options]", gsd_udp_send},
#endif    
};
...
int iperf_cli_init(void)
{
    ...
    if (ptimer == NULL)
        rv = os_timer_create(&ptimer, "UDP Poll Timer", pdMS_TO_TICKS(1U), timer_poll_udp_client, (void *)0,
