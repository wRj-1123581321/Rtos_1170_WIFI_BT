...
//SDK_ALIGN(uint8_t inbuf_arr[MAX_WMM_BUF_NUM][INBUF_WMM_LEN], BOARD_DATA_BUFFER_ALIGN_SIZE);
uint8_t __attribute__((aligned(BOARD_DATA_BUFFER_ALIGN_SIZE))) __attribute__((section(".gsd.$BOARD_SDRAM"))) inbuf_arr[MAX_WMM_BUF_NUM][INBUF_WMM_LEN];
...
#ifdef CONFIG_RX_POOL
static os_thread_stack_define(wifi_core_rx_stack, WIFI_CORE_STACK_SIZE);
#endif
...
int wifi_wait_for_cmdresp(void *cmd_resp_priv)
{
    ...
    ret = os_rwlock_read_lock(&sleep_rwlock, os_msec_to_ticks(MAX_WAIT_TIME));
    if (ret != WM_SUCCESS)
    {
#ifdef CONFIG_WIFI_PS_DEBUG
    ...
    ret = wifi_get_command_resp_sem(os_msec_to_ticks(WIFI_COMMAND_RESPONSE_WAIT_MS));
    if (ret != WM_SUCCESS)
    {
...
#ifdef CONFIG_RX_POOL
static void wifi_core_rx(void *argv)
{
    int sta;
	inbuf_t *pbuf = NULL;
    for (;;)
    {
		if (xQueueReceive(wm_wifi.io_rx, &pbuf, OS_WAIT_FOREVER)) {
			wlan_process_rx(mlan_adap, pbuf);
    	}
    } /* for ;; */
}
#endif
...
#ifdef CONFIG_RX_POOL
	wm_wifi.io_rx = xQueueCreate(MAX_WMM_BUF_NUM, sizeof(inbuf_t *));
#endif    
...
#ifdef CONFIG_RX_POOL
	ret = os_thread_create(&wm_wifi.wm_wifi_rx_thread, "wifi_core_rx", wifi_core_rx, NULL, &wifi_core_rx_stack,
                           OS_PRIO_1);

    if (ret != WM_SUCCESS)
    {
        wifi_e("Create wifi core rx thread failed");
        goto fail;
    }
#endif
...
#ifdef CONFIG_RX_POOL
	ret = wifi_wmm_inbuf_pool_init(&inbuf_arr[0][0]);
	if (ret != WM_SUCCESS)
	{
		wifi_e("Unable to init wmm input buffer pool");
		goto fail;
	}
...
void wifi_tx_card_awake_lock(void)
{
    ...
    ret = os_rwlock_read_lock(&sleep_rwlock, os_msec_to_ticks(MAX_WAIT_WAKEUP_TIME));
