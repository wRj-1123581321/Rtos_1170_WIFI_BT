...
inbuf_t *wifi_wmm_inbuf_get(void)
{
    inbuf_t *buf = MNULL;
	
	///PRINTF("----------1\r\n");
    mlan_adap->callbacks.moal_semaphore_get(mlan_adap->pmoal_handle, &mlan_adap->inbuf_pool.free_list.plock);

    assert(mlan_adap->inbuf_pool.free_cnt >= 0);

    if (mlan_adap->inbuf_pool.free_cnt == 0)
    {
        mlan_adap->callbacks.moal_semaphore_put(mlan_adap->pmoal_handle, &mlan_adap->inbuf_pool.free_list.plock);
        return MNULL;
    }

    buf = (inbuf_t *)util_dequeue_list(mlan_adap->pmoal_handle, &mlan_adap->inbuf_pool.free_list, MNULL, MNULL);
    assert(buf != MNULL);
    mlan_adap->inbuf_pool.free_cnt--;
...
void wifi_wmm_inbuf_put(inbuf_t *buf)
{
	//PRINTF("--------0\r\n");

    mlan_adap->callbacks.moal_semaphore_get(mlan_adap->pmoal_handle, &mlan_adap->inbuf_pool.free_list.plock);

    assert(mlan_adap->inbuf_pool.free_cnt < MAX_WMM_BUF_NUM);
...
//#endif
    ...
    util_enqueue_list_tail(mlan_adap->pmoal_handle, &mlan_adap->inbuf_pool.free_list, &buf->entry, MNULL, MNULL);
    mlan_adap->inbuf_pool.free_cnt++;

    mlan_adap->callbacks.moal_semaphore_put(mlan_adap->pmoal_handle, &mlan_adap->inbuf_pool.free_list.plock);
}

int wifi_wmm_inbuf_pool_init(uint8_t *pool)
{
    int i;
    inbuf_t *buf = MNULL;
	
	 __memset(mlan_adap, &mlan_adap->inbuf_pool, 0x00, sizeof(outbuf_pool_t));

    util_init_list_head(mlan_adap->pmoal_handle, &mlan_adap->inbuf_pool.free_list, MFALSE, MNULL);

    if (mlan_adap->callbacks.moal_init_semaphore(mlan_adap->pmoal_handle, "wmm_inbuf_pool_sem",
                                                 &mlan_adap->inbuf_pool.free_list.plock) != MLAN_STATUS_SUCCESS)
        return WM_E_NOMEM;
    ...
    for (i = 0; i < MAX_WMM_BUF_NUM; i++)
    {
        /* TODO: where to put buffer pool mgmt codes */
        // buf = (outbuf_t *)&outbuf_arr[i][0];
        buf = (inbuf_t *)(pool + (i * INBUF_WMM_LEN));
        util_init_list(&buf->entry);
        util_enqueue_list_tail(mlan_adap->pmoal_handle, &mlan_adap->inbuf_pool.free_list, &buf->entry, MNULL, MNULL);
    }
    mlan_adap->inbuf_pool.free_cnt = MAX_WMM_BUF_NUM;
	
    return WM_SUCCESS;
}
void wifi_wmm_inbuf_pool_deinit(void)
{
    mlan_adap->inbuf_pool.free_cnt = 0;

    mlan_adap->callbacks.moal_free_semaphore(mlan_adap->pmoal_handle, &mlan_adap->inbuf_pool.free_list.plock);

    util_free_list_head(mlan_adap->pmoal_handle, &mlan_adap->inbuf_pool.free_list, MNULL);
